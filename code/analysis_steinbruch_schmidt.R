...
ggplot(df, aes(y = he, x = t, color = type, group = group)) + 
  geom_rect(rect, inherit.aes = FALSE, mapping = aes(xmin = t_min, xmax = t_max, ymin = h_min, ymax = h_max)) +
  geom_line(aes(size = group)) +
  scale_size_manual(values = c("A" = env_lwd, "B" = env_lwd, "C" = med_lwd)) +
  scale_color_manual(values = c("blue", "red")) +
  xlab("Time [kyr]") +
  ylab("Stratigraphic position [m]") +
  ggtitle("Age-depth model") +
  annotate("text", x = mean(c(t_max, t_min)), y = mean(c(h_top_lkw, h_min)), label = "Hi") +
  annotate("text", x = mean(c(t_max, t_min)), y = mean(c(h_top_ukw, h_bottom_ukw)), label = "Hi") +
  geom_hline(yintercept = h_ashbed) +
  annotate("text", x = mean(c(t_max, t_min)), y = h_ashbed + 0.1, label = "Bentonite layer" )
...
plot_r2_opt(fa_prec, "R2opt_vs_sedrate", ".png")
...
plot_age_ff_boundary("age_ff_bdry", ".png")

# get the numbers

get_ff_bdr_nos = function(adm){
  l = sapply(admtools::get_time(x = adm_prec_abs, h = h_f_f_bdry ), function(x) x[1])
  ...
  return(list("mean" = ff_mean,
              "2sig" = ff_2sig,
              "median" = ff_median,
              "iqr" = ff_iqr,
              "quantile" = ff_quantile))
}

secc_ages = get_ff_bdr_nos(adm_secc_abs)

ff_mean_diff = ff_mean_dasilva - secc_ages$mean
ff_2sigma_diff = ff_2sigma_dasilva - secc_ages$`2sig`
ff_2sigma_rat = 1- secc_ages$`2sig` / ff_2sigma_dasilva
ff_mean_diff_da_silva = ff_mean_gradstein - ff_mean_dasilva
ff_mean_diff_gradstein = ff_mean_gradstein - secc_ages$mean
...
plot_duration_uke("duration_uke", ".png")

# some numbers
get_uke_duration = function(adm){
...
plot_elapsed_time_ke("time_elapsed_between_Kellwasser_events", ".png")


## some numbers on timing
get_time_between_ke = function(adm){
...
get_time_between_ke(adm_prec_abs)
